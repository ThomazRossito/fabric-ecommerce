...
# # 🥇 Gold: Dimensão Produtos (SCD Type 2)

# MARKDOWN ********************

# ## Descrição
# 
# Criação da dimensão de produtos para camada Gold com Slowly Changing Dimension Type 2.
# 
# ### Funcionalidades:
# 
# 1. **🔑 Surrogate Key** - Produto_SK (auto-incremento)
# 2. **📅 SCD Type 2** - Rastreamento de mudanças históricas
# 3. **🎯 Campos de Negócio** - Dados do produto e categoria
...
# |--------|------|----------|
# | produto_sk | BIGINT | Surrogate key (PK) |
# | produto_id | STRING | ID natural do produto |
# | nome | STRING | Nome do produto |
# | categoria | STRING | Categoria |
# | preco | DOUBLE | Preço atual |
# | faixa_preco | STRING | Baixo/Médio/Alto |
...
from pyspark.sql.functions import current_timestamp, lit, col, row_number, when
from pyspark.sql.window import Window
import time

print("="*30)
print("🥇 GOLD - DIMENSÃO PRODUTOS 🥇")
print("="*30)
...
df_silver = read_data(table_silver, "table")

# Adicionar Surrogate Key
window_spec = Window.orderBy("produto_id")
df_dim = df_silver.withColumn("produto_sk", row_number().over(window_spec))

param_options = {
    # Adicionar faixa de preço (Low/Medium/High)
    "faixa_preco": when(col("preco") < 100, "Baixo")
                  .when(col("preco") < 1000, "Médio")
                  .otherwise("Alto"),
...
# 3. Aplicamos tudo de uma única vez (sem os asteriscos **)
df_dim = df_dim.withColumns(param_options)

# Reordenar colunas
df_dim = df_dim.select(
    "produto_sk", "produto_id", "nome", 
    "categoria", "preco", "faixa_preco",
...
print(f"📊 Produtos únicos: {count_records:,}")
